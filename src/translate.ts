// 第三方赛果（ESPN）队名 / 联赛名中文化映射。
//
// 体彩官方接口（getMatchHeadV1 等）本身返回中文队名，无需映射；本文件仅用于
// 把 ESPN 免费赛果接口返回的葡/西/英等外文队名翻译成中文，让竞彩工具显示更直观。
//
// 设计要点：
// - 仅在前端「展示层」翻译，后端 / API 数据保持原始外文不变。原因：后端
//   insight.js 的 ESPN 战绩 / 交锋判定依赖原始队名做主客匹配，若改成中文会导致
//   主客识别与胜负结果算反。前端传原始名给后端、显示时再翻译，两全其美。
// - 未收录的冷门队名回退原名（便于核对原始数据）。
// - 匹配时做归一化（小写 + 去变音符号），兼容 Vitória / Vitoria 这类写法差异。

function normalize(s: string): string {
  return (s || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '') // 去掉葡/西/法等语言的重音符号
}

// 联赛代码 -> 中文名
export const LEAGUE_ZH: Record<string, string> = {
  'eng.1': '英超',
  'esp.1': '西甲',
  'ita.1': '意甲',
  'ger.1': '德甲',
  'fra.1': '法甲',
  'uefa.champions': '欧冠',
  'uefa.europa': '欧联',
  'bra.1': '巴甲',
  'arg.1': '阿甲',
  'mex.1': '墨超',
  'chn.1': '中超',
  'jpn.1': '日职',
  'kor.1': '韩K',
  'conmebol.libertadores': '解放者杯',
  'por.1': '葡超',
  'ned.1': '荷甲',
  'sco.1': '苏超',
  'tur.1': '土超',
  'bel.1': '比甲',
  'usa.1': '美职联',
  'eng.2': '英冠',
  'esp.2': '西乙',
  'ger.2': '德乙',
  'ita.2': '意乙',
  'fra.2': '法乙',
}

// 队名（原始外文）-> 中文。键用原始写法即可，匹配时会被归一化。
const TEAM_ZH: Record<string, string> = {
  // ---------------- 英超 ----------------
  'Manchester City': '曼城',
  'Arsenal': '阿森纳',
  'Liverpool': '利物浦',
  'Chelsea': '切尔西',
  'Manchester United': '曼联',
  'Tottenham Hotspur': '热刺',
  'Tottenham': '热刺',
  'Newcastle United': '纽卡斯尔联',
  'Newcastle': '纽卡斯尔联',
  'Aston Villa': '阿斯顿维拉',
  'Brighton & Hove Albion': '布莱顿',
  'Brighton': '布莱顿',
  'West Ham United': '西汉姆联',
  'West Ham': '西汉姆联',
  'Everton': '埃弗顿',
  'Leicester City': '莱斯特城',
  'Crystal Palace': '水晶宫',
  'Brentford': '布伦特福德',
  'Fulham': '富勒姆',
  'Wolverhampton Wanderers': '狼队',
  'Wolves': '狼队',
  'AFC Bournemouth': '伯恩茅斯',
  'Bournemouth': '伯恩茅斯',
  'Nottingham Forest': '诺丁汉森林',
  'Leeds United': '利兹联',
  'Southampton': '南安普顿',
  'Ipswich Town': '伊普斯维奇',
  'Norwich City': '诺维奇',
  'Watford': '沃特福德',
  'West Bromwich Albion': '西布朗',
  'Middlesbrough': '米德尔斯堡',
  'Sheffield United': '谢菲尔德联',
  'Sheffield Wednesday': '谢周三',
  'Sunderland': '桑德兰',
  'Coventry City': '考文垂',
  'Blackburn Rovers': '布莱克本',
  'Millwall': '米尔沃尔',
  'Queens Park Rangers': '女王公园巡游者',
  'QPR': '女王公园巡游者',
  'Stoke City': '斯托克城',
  'Preston North End': '普雷斯顿',
  'Bristol City': '布里斯托尔城',
  'Swansea City': '斯旺西',
  'Cardiff City': '卡迪夫城',
  'Hull City': '赫尔城',
  'Plymouth Argyle': '普利茅斯',
  'Rotherham United': '罗瑟勒姆',

  // ---------------- 西甲 ----------------
  'Real Madrid': '皇家马德里',
  'Barcelona': '巴塞罗那',
  'Atlético Madrid': '马德里竞技',
  'Atletico Madrid': '马德里竞技',
  'Sevilla': '塞维利亚',
  'Athletic Club': '毕尔巴鄂竞技',
  'Bilbao': '毕尔巴鄂竞技',
  'Valencia': '瓦伦西亚',
  'Villarreal': '比利亚雷亚尔',
  'Real Sociedad': '皇家社会',
  'Real Betis': '皇家贝蒂斯',
  'Girona': '赫罗纳',
  'Osasuna': '奥萨苏纳',
  'Celta Vigo': '塞尔塔',
  'Getafe': '赫塔菲',
  'RCD Espanyol': '西班牙人',
  'Espanyol': '西班牙人',
  'Mallorca': '马洛卡',
  'Rayo Vallecano': '巴列卡诺',
  'Las Palmas': '拉斯帕尔马斯',
  'Alavés': '阿拉维斯',
  'Alaves': '阿拉维斯',
  'Valladolid': '巴拉多利德',
  'Leganés': '莱加内斯',
  'Leganes': '莱加内斯',
  'Cádiz': '加的斯',
  'Cadiz': '加的斯',
  'Granada': '格拉纳达',

  // ---------------- 意甲 ----------------
  'Inter': '国际米兰',
  'Internazionale': '国际米兰',
  'AC Milan': 'AC米兰',
  'Milan': 'AC米兰',
  'Juventus': '尤文图斯',
  'Napoli': '那不勒斯',
  'Roma': '罗马',
  'Lazio': '拉齐奥',
  'Atalanta': '亚特兰大',
  'Fiorentina': '佛罗伦萨',
  'Bologna': '博洛尼亚',
  'Torino': '都灵',
  'Monza': '蒙扎',
  'Lecce': '莱切',
  'Sassuolo': '萨索洛',
  'Salernitana': '萨勒尼塔纳',
  'Empoli': '恩波利',
  'Genoa': '热那亚',
  'Sampdoria': '桑普多利亚',
  'Udinese': '乌迪内斯',
  'Hellas Verona': '维罗纳',
  'Verona': '维罗纳',
  'Cagliari': '卡利亚里',
  'Parma': '帕尔马',
  'Como': '科莫',
  'Venezia': '威尼斯',
  'Spezia': '斯佩齐亚',
  'Frosinone': '弗罗西诺内',

  // ---------------- 德甲 ----------------
  'Bayern Munich': '拜仁慕尼黑',
  'Bayern': '拜仁慕尼黑',
  'Borussia Dortmund': '多特蒙德',
  'Dortmund': '多特蒙德',
  'Bayer Leverkusen': '勒沃库森',
  'Leverkusen': '勒沃库森',
  'RB Leipzig': 'RB莱比锡',
  'Leipzig': 'RB莱比锡',
  'Eintracht Frankfurt': '法兰克福',
  'Frankfurt': '法兰克福',
  'Union Berlin': '柏林联合',
  'VfB Stuttgart': '斯图加特',
  'Stuttgart': '斯图加特',
  'Borussia Mönchengladbach': '门兴格拉德巴赫',
  'Borussia Monchengladbach': '门兴格拉德巴赫',
  'Mönchengladbach': '门兴格拉德巴赫',
  'VfL Wolfsburg': '沃尔夫斯堡',
  'Wolfsburg': '沃尔夫斯堡',
  'SC Freiburg': '弗赖堡',
  'Freiburg': '弗赖堡',
  'TSG Hoffenheim': '霍芬海姆',
  'Hoffenheim': '霍芬海姆',
  'Mainz 05': '美因茨',
  'Mainz': '美因茨',
  'Werder Bremen': '云达不莱梅',
  'Bremen': '云达不莱梅',
  'FC Köln': '科隆',
  'Köln': '科隆',
  'Cologne': '科隆',
  'Hertha BSC': '柏林赫塔',
  'Hertha': '柏林赫塔',
  'FC Augsburg': '奥格斯堡',
  'Augsburg': '奥格斯堡',
  'Heidenheim': '海登海姆',
  'VfL Bochum': '波鸿',
  'Bochum': '波鸿',
  'Darmstadt 98': '达姆施塔特',
  'Darmstadt': '达姆施塔特',
  'Hamburg SV': '汉堡',
  'Hamburger SV': '汉堡',
  'Greuther Fürth': '菲尔特',
  'Greuther Furth': '菲尔特',
  'Karlsruher SC': '卡尔斯鲁厄',
  'Hannover 96': '汉诺威96',
  'Fortuna Düsseldorf': '杜塞尔多夫',
  'Fortuna Dusseldorf': '杜塞尔多夫',
  'SV Elversberg': '埃弗斯贝格',
  'SC Paderborn': '帕德博恩',
  '1. FC Nürnberg': '纽伦堡',
  'Nurnberg': '纽伦堡',
  'FC Schalke 04': '沙尔克04',
  'Schalke': '沙尔克04',
  'Wehen Wiesbaden': '韦恩',

  // ---------------- 法甲 ----------------
  'Paris Saint-Germain': '巴黎圣日耳曼',
  'Paris SG': '巴黎圣日耳曼',
  'PSG': '巴黎圣日耳曼',
  'Marseille': '马赛',
  'Olympique de Marseille': '马赛',
  'Monaco': '摩纳哥',
  'AS Monaco': '摩纳哥',
  'Lyon': '里昂',
  'Olympique Lyonnais': '里昂',
  'Lille': '里尔',
  'Nice': '尼斯',
  'OGC Nice': '尼斯',
  'Rennes': '雷恩',
  'Lens': '朗斯',
  'Saint-Étienne': '圣埃蒂安',
  'Saint-Etienne': '圣埃蒂安',
  'Nantes': '南特',
  'Strasbourg': '斯特拉斯堡',
  'Montpellier': '蒙彼利埃',
  'Reims': '兰斯',
  'Toulouse': '图卢兹',
  'Brest': '布雷斯特',
  'Auxerre': '欧塞尔',
  'Clermont': '克莱蒙',
  'Le Havre': '勒阿弗尔',
  'Metz': '梅斯',
  'Bordeaux': '波尔多',

  // ---------------- 巴甲 ----------------
  'Flamengo': '弗拉门戈',
  'Corinthians': '科林蒂安',
  'Palmeiras': '帕尔梅拉斯',
  'São Paulo': '圣保罗',
  'Santos': '桑托斯',
  'Vasco da Gama': '瓦斯科达伽马',
  'Fluminense': '弗鲁米嫩塞',
  'Botafogo': '博塔弗戈',
  'Athletico-PR': '巴拉纳竞技',
  'Athletico Paranaense': '巴拉纳竞技',
  'Atlético-PR': '巴拉纳竞技',
  'Grêmio': '格雷米奥',
  'Internacional': '国际队',
  'Cruzeiro': '克鲁塞罗',
  'Atlético Mineiro': '米内罗竞技',
  'Atletico Mineiro': '米内罗竞技',
  'Bahia': '巴伊亚',
  'Fortaleza': '福塔雷萨',
  'Red Bull Bragantino': '红牛布拉甘蒂诺',
  'Bragantino': '红牛布拉甘蒂诺',
  'Cuiabá': '库亚巴',
  'Cuiaba': '库亚巴',
  'Coritiba': '科里蒂巴',
  'Goiás': '戈亚斯',
  'Goias': '戈亚斯',
  'Vitória': '维多利亚',
  'Vitoria': '维多利亚',
  'Juventude': '尤文图德',
  'Criciúma': '克里西乌马',
  'Criciuma': '克里西乌马',
  'América Mineiro': '米内罗美洲',
  'America Mineiro': '米内罗美洲',
  'Sport Recife': '累西腓体育',
  'Ceará': '塞阿拉',
  'Ceara': '塞阿拉',
  'Atlético-GO': '戈亚尼亚竞技',
  'Atletico-GO': '戈亚尼亚竞技',
  'Atlético Goianiense': '戈亚尼亚竞技',
  'Mirassol': '米拉索尔',

  // ---------------- 阿甲 ----------------
  'River Plate': '河床',
  'Boca Juniors': '博卡青年',
  'Racing Club': '竞技',
  'Racing': '竞技',
  'Independiente': '独立',
  'San Lorenzo': '圣洛伦索',
  'Huracán': '飓风',
  'Huracan': '飓风',
  'Vélez Sarsfield': '萨斯菲尔德',
  'Velez Sarsfield': '萨斯菲尔德',
  'Estudiantes': '学生队',
  'Lanús': '拉努斯',
  'Lanus': '拉努斯',
  'Defensa y Justicia': '防御与正义',
  'Defensa': '防御与正义',
  'Talleres': '塔列雷斯',
  'Godoy Cruz': '戈多伊克鲁斯',
  'Unión': '联盟',
  'Union': '联盟',
  'Newell’s Old Boys': '纽维尔老男孩',
  'Newell\'s Old Boys': '纽维尔老男孩',
  'Rosario Central': '罗萨里奥中央',
  'Gimnasia La Plata': '拉普拉塔体操',
  'Gimnasia': '拉普拉塔体操',
  'Barracas Central': '巴拉卡斯中央',
  'Argentinos Juniors': '阿根廷青年人',
  'Banfield': '班菲尔德',
  'Colón': '科隆',
  'Colon': '科隆',
  'Central Córdoba': '科尔多瓦中央',
  'Central Cordoba': '科尔多瓦中央',
  'Sarmiento': '萨米恩托',
  'Platense': '普拉滕斯',
  'Tigre': '老虎',
  'San Martín': '圣马丁',
  'San Martin': '圣马丁',
  'Aldosivi': '阿尔多西维',
  'Instituto': '学院',
  'Atlético Tucumán': '图库曼竞技',
  'Atletico Tucuman': '图库曼竞技',
  'Belgrano': '贝尔格拉诺',
  'Independiente Rivadavia': '独立里瓦达维亚',
  'San Martín de San Juan': '圣胡安圣马丁',

  // ---------------- 葡超 ----------------
  'Benfica': '本菲卡',
  'Porto': '波尔图',
  'Sporting CP': '葡萄牙体育',
  'Sporting Lisbon': '葡萄牙体育',
  'Sporting': '葡萄牙体育',
  'Braga': '布拉加',
  'Vitória SC': '吉马良斯',
  'Vitoria SC': '吉马良斯',
  'Vitória de Guimarães': '吉马良斯',
  'Vitoria de Guimaraes': '吉马良斯',
  'Guimarães': '吉马良斯',
  'Boavista': '博阿维斯塔',
  'Moreirense': '莫雷拉伦斯',
  'Gil Vicente': '吉维森特',
  'Rio Ave': '里奥阿维',
  'Santa Clara': '圣克拉拉',
  'Famalicão': '法马利康',
  'Famalicao': '法马利康',
  'Casa Pia': '卡萨皮亚',
  'Estoril': '埃斯托里尔',
  'Arouca': '阿罗卡',
  'Farense': '法伦斯',
  'Estrela Amadora': '阿马多拉之星',
  'Nacional': '国民',
  'AVS': '阿维斯',
  'Académico de Viseu': '维塞乌学院',
  'Academico de Viseu': '维塞乌学院',
  'Chaves': '沙维什',
  'Paços de Ferreira': '费雷拉',
  'Pacos de Ferreira': '费雷拉',
  'Portimonense': '波尔蒂芒人',
  'Vizela': '维泽拉',
  'Marítimo': '马里迪莫',
  'Maritimo': '马里迪莫',
  'Varzim': '瓦尔津',

  // ---------------- 中超 ----------------
  'Shanghai Port': '上海海港',
  'Shanghai Shenhua': '上海申花',
  'Beijing Guoan': '北京国安',
  'Shandong Taishan': '山东泰山',
  'Chengdu Rongcheng': '成都蓉城',
  'Wuhan Three Towns': '武汉三镇',
  'Zhejiang': '浙江队',
  'Henan': '河南队',
  'Tianjin Jinmen Tiger': '天津津门虎',
  'Cangzhou Mighty Lions': '沧州雄狮',
  'Cangzhou': '沧州雄狮',
  'Meizhou Hakka': '梅州客家',
  'Shenzhen': '深圳队',
  'Dalian Yingbo': '大连英博',
  'Dalian': '大连人',
  'Nantong Zhiyun': '南通支云',
  'Qingdao': '青岛队',
  'Changchun Yatai': '长春亚泰',
  'Wuhan': '武汉队',
  'Hunan': '湖南队',
  'Yunnan Yukun': '云南玉昆',
  'Qingdao West Coast': '青岛西海岸',
  'Chengdu': '成都蓉城',

  // ---------------- 日职 ----------------
  'Vissel Kobe': '神户胜利船',
  'Yokohama F. Marinos': '横滨水手',
  'Yokohama Marinos': '横滨水手',
  'Kawasaki Frontale': '川崎前锋',
  'Urawa Red Diamonds': '浦和红钻',
  'Urawa Reds': '浦和红钻',
  'Gamba Osaka': '大阪钢巴',
  'Cerezo Osaka': '大阪樱花',
  'FC Tokyo': '东京FC',
  'Sanfrecce Hiroshima': '广岛三箭',
  'Nagoya Grampus': '名古屋鲸',
  'Kashima Antlers': '鹿岛鹿角',
  'Tokyo Verdy': '东京绿茵',
  'Kyoto Sanga': '京都桑加',
  'Avispa Fukuoka': '福冈黄蜂',
  'Shonan Bellmare': '湘南海洋',
  'Machida Zelvia': '町田泽维亚',
  'Albirex Niigata': '新泻天鹅',
  'Jubilo Iwata': '磐田喜悦',
  'Kashiwa Reysol': '柏太阳神',
  'Sagan Tosu': '鸟栖沙岩',
  'Consadole Sapporo': '札幌冈萨多',

  // ---------------- 韩K ----------------
  'Ulsan Hyundai': '蔚山现代',
  'Ulsan': '蔚山现代',
  'Jeonbuk Hyundai Motors': '全北现代',
  'Jeonbuk': '全北现代',
  'FC Seoul': '首尔FC',
  'Pohang Steelers': '浦项制铁',
  'Daegu FC': '大邱FC',
  'Suwon Samsung Bluewings': '水原三星',
  'Incheon United': '仁川联',
  'Gwangju FC': '光州FC',
  'Jeju United': '济州联',
  'Gangwon FC': '江原FC',
  'Daejeon Hana Citizen': '大田市民',
  'Gimcheon Sangmu': '金泉尚武',
  'Kim Cheon': '金泉尚武',
  'Suwon FC': '水原FC',

  // ---------------- 墨超 ----------------
  'Club América': '美洲',
  'América': '美洲',
  'America': '美洲',
  'Cruz Azul': '蓝十字',
  'Guadalajara': '瓜达拉哈拉',
  'Chivas': '瓜达拉哈拉',
  'Tigres UANL': '老虎大学',
  'Tigres': '老虎大学',
  'Monterrey': '蒙特雷',
  'Pumas UNAM': '美洲狮',
  'Pumas': '美洲狮',
  'Pachuca': '帕丘卡',
  'Toluca': '托卢卡',
  'Santos Laguna': '桑托斯',
  'León': '莱昂',
  'Leon': '莱昂',
  'Atlas': '阿特拉斯',
  'Necaxa': '内卡萨',
  'Querétaro': '克雷塔罗',
  'Queretaro': '克雷塔罗',
  'Mazatlán': '马萨特兰',
  'Mazatlan': '马萨特兰',
  'Puebla': '普埃布拉',
  'Juárez': '华雷斯',
  'Juarez': '华雷斯',
  'Tijuana': '蒂华纳',
  'Atlético San Luis': '圣路易斯竞技',
  'Atletico San Luis': '圣路易斯竞技',

  // ---------------- 荷甲 ----------------
  'Ajax': '阿贾克斯',
  'PSV Eindhoven': '埃因霍温',
  'PSV': '埃因霍温',
  'Feyenoord': '费耶诺德',
  'AZ Alkmaar': '阿尔克马尔',
  'AZ': '阿尔克马尔',
  'Twente': '特温特',
  'FC Twente': '特温特',
  'Utrecht': '乌得勒支',
  'Vitesse': '维特斯',
  'SC Heerenveen': '海伦芬',
  'Groningen': '格罗宁根',
  'Go Ahead Eagles': '前进之鹰',
  'NEC Nijmegen': '奈梅亨',
  'Sparta Rotterdam': '鹿特丹斯巴达',
  'RKC Waalwijk': '瓦尔韦克',
  'Fortuna Sittard': '福图纳',
  'Excelsior': '精英',
  'Heracles': '赫拉克勒斯',
  'Cambuur': '坎布尔',
  'Emmen': '埃门',

  // ---------------- 苏超 ----------------
  'Celtic': '凯尔特人',
  'Rangers': '流浪者',
  'Aberdeen': '阿伯丁',
  'Hearts': '哈茨',
  'Heart of Midlothian': '哈茨',
  'Hibernian': '希伯尼安',
  'Hibs': '希伯尼安',
  'St Johnstone': '圣约翰斯顿',
  'Motherwell': '马瑟韦尔',
  'Kilmarnock': '基尔马诺克',
  'St Mirren': '圣米伦',
  'Livingston': '利文斯顿',
  'Ross County': '罗斯郡',
  'Dundee': '邓迪',
  'Dundee United': '邓迪联',
  'Partick Thistle': '帕尔蒂克',

  // ---------------- 土超 ----------------
  'Galatasaray': '加拉塔萨雷',
  'Fenerbahçe': '费内巴切',
  'Fenerbahce': '费内巴切',
  'Beşiktaş': '贝西克塔斯',
  'Besiktas': '贝西克塔斯',
  'Trabzonspor': '特拉布宗体育',
  'Başakşehir': '巴沙克谢希尔',
  'Basaksehir': '巴沙克谢希尔',
  'Adana Demirspor': '阿达纳',
  'Konyaspor': '科尼亚',
  'Kayserispor': '开塞利',
  'Sivasspor': '锡瓦斯',
  'Alanyaspor': '阿兰亚',
  'Antalyaspor': '安塔利亚',
  'Rizespor': '里泽',
  'Kasimpasa': '卡辛帕沙',
  'KasımPaşa': '卡辛帕沙',
  'Kasımpaşa': '卡辛帕沙',
  'Hatayspor': '哈塔伊',
  'Gaziantep': '加济安泰普',
  'Ankaragücü': '安卡拉古库',
  'Ankaragucu': '安卡拉古库',
  'Samsunspor': '萨姆松',
  'Fatih Karagümrük': '卡拉古鲁克',
  'Pendikspor': '彭迪克',
  'Istanbulspor': '伊斯坦布尔',
  'Eyüpspor': '埃于普',
  'Kocaelispor': '科贾埃利',

  // ---------------- 比甲 ----------------
  'Anderlecht': '安德莱赫特',
  'Club Brugge': '布鲁日',
  'Brugge': '布鲁日',
  'KRC Genk': '亨克',
  'Genk': '亨克',
  'Union Saint-Gilloise': '圣吉罗斯联',
  'Union SG': '圣吉罗斯联',
  'Antwerp': '安特卫普',
  'Royal Antwerp': '安特卫普',
  'Gent': '根特',
  'AA Gent': '根特',
  'Standard Liège': '标准列日',
  'Standard': '标准列日',
  'Westerlo': '韦斯特洛',
  'Cercle Brugge': '布鲁日塞尔克',
  'Mechelen': '梅赫伦',
  'Sint-Truiden': '圣特吕登',
  'OH Leuven': '奥哈瓦里',
  'Charleroi': '沙勒罗瓦',
  'Kortrijk': '科尔特里克',
  'RWD Molenbeek': '莫伦贝克',

  // ---------------- 美职联 ----------------
  'Inter Miami': '国际迈阿密',
  'Inter Miami CF': '国际迈阿密',
  'LA Galaxy': '洛杉矶银河',
  'LAFC': '洛杉矶FC',
  'Los Angeles FC': '洛杉矶FC',
  'Seattle Sounders': '西雅图海湾人',
  'Atlanta United': '亚特兰大联',
  'New York City FC': '纽约城',
  'NYCFC': '纽约城',
  'Toronto FC': '多伦多FC',
  'CF Montréal': '蒙特利尔',
  'CF Montreal': '蒙特利尔',
  'Philadelphia Union': '费城联合',
  'FC Cincinnati': '辛辛那提',
  'Nashville SC': '纳什维尔',
  'New England Revolution': '新英格兰革命',
  'Portland Timbers': '波特兰伐木者',
  'Real Salt Lake': '皇家盐湖城',
  'Houston Dynamo': '休斯顿迪纳摩',
  'FC Dallas': '达拉斯FC',
  'Sporting Kansas City': '堪萨斯城体育',
  'Minnesota United': '明尼苏达联',
  'Vancouver Whitecaps': '温哥华白帽',
  'Colorado Rapids': '科罗拉多急流',
  'DC United': '华盛顿联',
  'Chicago Fire': '芝加哥火焰',
  'Columbus Crew': '哥伦布机员',
  'Charlotte FC': '夏洛特FC',
  'Austin FC': '奥斯汀FC',
  'St. Louis CITY': '圣路易斯',
  'San Jose Earthquakes': '圣何塞地震',
  'Orlando City': '奥兰多城',

  // ---------------- 国家队（部分赛事会出现） ----------------
  'Argentina': '阿根廷',
  'France': '法国',
  'Brazil': '巴西',
  'England': '英格兰',
  'Portugal': '葡萄牙',
  'Spain': '西班牙',
  'Germany': '德国',
  'Netherlands': '荷兰',
  'Belgium': '比利时',
  'Croatia': '克罗地亚',
  'Italy': '意大利',
  'Uruguay': '乌拉圭',
}

const teamMap = new Map<string, string>()
for (const [k, v] of Object.entries(TEAM_ZH)) {
  teamMap.set(normalize(k), v)
}

/** 把原始外文队名翻译成中文；未收录则回退原名。 */
export function zhTeam(name?: string | null): string {
  if (!name) return ''
  return teamMap.get(normalize(name)) || name
}

/** 联赛对象 -> 中文联赛名（按 code 映射，回退到原名）。 */
export function zhLeagueName(lg?: { code?: string; abbName?: string; allName?: string } | null): string {
  if (!lg) return ''
  if (lg.code) {
    const z = LEAGUE_ZH[lg.code]
    if (z) return z
  }
  return lg.abbName || lg.allName || ''
}
