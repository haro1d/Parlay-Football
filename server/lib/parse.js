...
export function parseUpstream(payload, opts = {}) {
  ...
  for (const group of groups) {
    ...
    for (const sm of group.subMatchList ?? []) {
      ...
      if (leagueQuery) {
        const hay =
          `${str(sm.leagueCode)} ${str(sm.leagueAbbName)} ${str(sm.leagueAllName)}`.toLowerCase()
        if (!hay.includes(leagueQuery)) continue
      }
      if (kw) {
        const hay =
          `${str(sm.homeTeamAbbName)} ${str(sm.homeTeamAllName)} ${str(sm.awayTeamAbbName)} ${str(sm.awayTeamAllName)}`.toLowerCase()
  ...
  if (opts.extraMatches?.length) {
    for (const em of opts.extraMatches) {
      if (leagueQuery) {
        const hay =
          `${str(em.league?.code)} ${str(em.league?.abbName)} ${str(em.league?.allName)}`.toLowerCase()
        if (!hay.includes(leagueQuery)) continue
      }
      if (kw) {
        const hay =
          `${str(em.home?.abbName)} ${str(em.home?.allName)} ${str(em.away?.abbName)} ${str(em.away?.allName)}`.toLowerCase()
        if (!hay.includes(kw)) continue
      }
      matches.push(em)
    }
  ...
  for (const m of matches) {
    ...
    if (!leagueMap.has(key)) {
      leagueMap.set(key, {
        code: m.league.code,
        abbName: m.league.abbName,
        allName: m.league.allName,
      })
    }
  }
  const leagues = Array.from(leagueMap.values()).sort((a, b) =>
    a.abbName.localeCompare(b.abbName, 'zh'),
  )

  // 体彩官方排序：同一天内按 matchNumStr 序号升序（=体彩 app 展示顺序，如"周三001"<"周三002"）。
  // 注意：matchId 升序 ≠ 序号升序！体彩同日内序号小的比赛可能 matchId 更大（如序号001的 matchId
  // 反而最大），故在售比赛必须用 matchNumStr 排序才能对上体彩 app。
  // 已完赛比赛来自 getFixedBonusV1，无 matchNumStr，退化为按 matchId 升序。跨日按日期方向。
  const tOf = (m) => m.matchTime || '00:00'
  // dateDir: 1=日期升序, -1=日期降序；同日内优先体彩序号(matchNumStr)，无则按 matchId
  const byDateSeq = (dateDir) => (a, b) => {
    if (a.matchDate !== b.matchDate) return (a.matchDate < b.matchDate ? -1 : 1) * dateDir
    const na = a.matchNumStr || ''
    const nb = b.matchNumStr || ''
    if (na && nb) {
      if (na !== nb) return na < nb ? -1 : 1
    } else if (na !== nb) {
      // 一方有序号一方无：有序号的靠前（在售优先于已完赛回退排序）
      return na ? -1 : 1
    }
